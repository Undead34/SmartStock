...
export default function Register() {
  ...
  useEffect(() => {
    const unsubscribe = window.SmartStock.receive(
      "smartstock:register:user",
      (event, data) => {
        switch (data.CODE) {
          case "ALREADY_EXISTS":
  ...
  function handlerSubmit(event) {
    ...
    window.SmartStock.send("smartstock:register:user", {
      name,
      email,
      password,
    });
    ...
    <div className="relative w-full h-full flex flex-col justify-center items-center bg-[#f2f6ff] text-white gap-2">
      <div className="p-10 flex flex-col bg-[#ffffff] w-[32rem] shadow-xl rounded-lg">
        <div className="flex flex-col">
          <div className="text-center mb-8">
            <div className="text-2xl text-[#1c7cb4] font-semibold mb-2">
              ¡Bienvenido, futuro usuario!
            </div>
            <div
              className={`text-sm text-gray-400 transition-all ${
                error
                  ? error === "¡Todo listo!"
                    ? "bg-[#0fff6322] rounded-sm border border-solid border-[#00ff0d0f] text-[#6e6f72]"
                    : "bg-[#ff0f0f22] border border-solid border-[#ff00000f] text-[#6e6f72]"
                  : ""
              }`}
            >
          ...
          <div className="flex flex-col">
            <form className="flex flex-col" onSubmit={handlerSubmit}>
              <div className="flex flex-col gap-6 mb-4 text-[#1c7cb4]">
                ...
                <div className="flex flex-row-reverse h-10 gap-2 rounded-md p-4 bg-white justify-start items-center border border-[#f9661111]">
                  <input
                    ...
                    placeholder="Por razones de seguridad, elija una contraseña"
                  />
                  {showPassword ? (
                    <AiFillEyeInvisible
                      onClick={() => {
                      ...
                      className="text-[#f96611] cursor-pointer"
                    />
                  ) : (
                    <AiFillEye
                      onClick={() => {
                ...
                <div className="flex flex-row-reverse h-10 gap-2 rounded-md p-4 bg-white justify-start items-center border border-[#f9661111]">
                  <input
                    ...
                    placeholder="Confirme su contraseña ingresándola nuevamente"
                  />
                  {showRePassword ? (
                    <AiFillEyeInvisible
                      onClick={() => {
              ...
              <div className="flex items-center justify-center mb-2">
                <button className="flex-1 bg-[#f96611] text-xl rounded-md p-2">
                  Registrarse
                </button>
              </div>
            </form>
      ...
      <div className="flex flex-col bg-[#ffffff] w-[32rem] shadow-xl rounded-lg p-2 text-center">
        <span className="text-gray-400">
          ¿Ya tiene cuenta?{" "}
          <Link to={"/login"} className="text-[#1c7cb4] underline">
            Iniciar sesión
          </Link>
        </span>
      </div>
